-- Demo Data for Mini ERP System (Railway version)
...
UPDATE chart_of_accounts SET balance = 12500.00 WHERE code = '5200'; -- Operating Expenses
UPDATE chart_of_accounts SET balance = 9500.00 WHERE code = '5500'; -- Administrative
...
('Downtown Office Complex', 'Modern 12-story office building in downtown financial district', 500000, 285000, 55),
('Residential Tower A', 'Luxury 20-floor residential tower with amenities', 800000, 420000, 48),
('Shopping Mall Renovation', 'Complete renovation of 3-level shopping center', 350000, 98000, 32),
('Green Housing Development', 'Eco-friendly residential community project', 650000, 112000, 18),
('Urban Park Infrastructure', 'Public park development with recreational facilities', 280000, 245000, 92);

-- Add demo invoices
INSERT INTO invoices (project_id, customer_id, description, amount, status, issued_at) VALUES
(1, 1, 'Phase 1 - Foundation Work', 95000, 'paid', '2024-02-01'),
(1, 1, 'Phase 2 - Structural Framework', 120000, 'paid', '2024-04-01'),
(1, 1, 'Phase 3 - Interior Work', 70000, 'pending', '2024-11-01'),
(2, 2, 'Initial Construction Payment', 180000, 'paid', '2024-05-01'),
(2, 2, 'Progress Payment - Floors 1-5', 150000, 'paid', '2024-08-01'),
(2, 2, 'Progress Payment - Floors 6-8', 90000, 'pending', '2024-11-15'),
(3, 3, 'Demolition & Site Prep', 60000, 'paid', '2024-07-01'),
(3, 3, 'Renovation Materials', 38000, 'pending', '2024-10-01'),
(4, 4, 'Planning & Permits', 75000, 'paid', '2024-10-01'),
(5, 5, 'Infrastructure Phase 1', 140000, 'paid', '2024-01-15'),
(5, 5, 'Infrastructure Phase 2 - Final', 105000, 'paid', '2024-06-01');

-- Add sample journal entries
INSERT INTO journal_entries (entry_number, entry_date, description, reference, status, created_by) VALUES
('JE-2024-001', '2024-01-15', 'Initial Capital Investment', 'CAP-001', 'approved', 1),
('JE-2024-002', '2024-02-01', 'Equipment Purchase', 'EQ-001', 'approved', 1),
('JE-2024-003', '2024-03-15', 'Revenue Recognition - Project 1', 'INV-2024-001', 'approved', 1),
('JE-2024-004', '2024-11-01', 'Monthly Expense Accrual', 'EXP-NOV', 'draft', 1);

-- Journal entry lines for balanced entries
-- Entry 1: Capital Investment ($350,000)
INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description) VALUES
(1, (SELECT id FROM chart_of_accounts WHERE code = '1000'), 350000, 0, 'Cash received from owner'),
(1, (SELECT id FROM chart_of_accounts WHERE code = '3000'), 0, 350000, 'Owner equity contribution');

-- Entry 2: Equipment Purchase ($85,000)
INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description) VALUES
(2, (SELECT id FROM chart_of_accounts WHERE code = '1500'), 85000, 0, 'New construction equipment'),
(2, (SELECT id FROM chart_of_accounts WHERE code = '1000'), 0, 85000, 'Cash payment for equipment');

-- Entry 3: Revenue Recognition ($95,000)
INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description) VALUES
(3, (SELECT id FROM chart_of_accounts WHERE code = '1100'), 95000, 0, 'Accounts receivable'),
(3, (SELECT id FROM chart_of_accounts WHERE code = '4100'), 0, 95000, 'Project revenue earned');

-- Entry 4: Expense Accrual (Draft - $15,000)
INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description) VALUES
(4, (SELECT id FROM chart_of_accounts WHERE code = '5500'), 15000, 0, 'November admin expenses'),
(4, (SELECT id FROM chart_of_accounts WHERE code = '2000'), 0, 15000, 'Accounts payable accrual');
